...
- **Losing a race is `SLOT_UNAVAILABLE`, never `VALIDATION_FAILED`.** `holdSlot()` re-derives the requested slot from the clinic's rules before inserting, and asks two questions separately: *would this clinic ever offer this start* (checked against the working windows with no occupancy at all — a "no" is a validation error, and retrying will not help) and *is that slot free right now* (a "no" is `SLOT_UNAVAILABLE`, and the agent's move is to offer another time). Folding the two together made a caller who merely lost a race to a competing transaction look as though it had sent a malformed request.
...
- **A failed `book()` or `reschedule()` still commits its cleanup.** When the hold turns out to have expired, the transaction deletes it and returns the failure as a *value*; the error is raised after the commit. Throwing from inside would roll that delete back and leave a dead row blocking the slot until the next sweep.
